...
async def async_setup_entry(
    ...
    # LightControllerV2 presence detection switch
    for lc in get_all(loxconfig, "LightControllerV2"):
        lc = add_room_and_cat_to_value_values(loxconfig, lc)
        if lc.get("presence", None):
            presence_kwargs = {
                **lc,
                "type": "LightPresenceSwitch",
                "presenceUuid": lc["states"]["presence"],
            }
            entities.append(LoxoneLightPresenceSwitch(**presence_kwargs))
...
class LoxoneLightPresenceSwitch(LoxoneEntity, SwitchEntity):
